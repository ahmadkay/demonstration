		...
		<body>	
		 <header id="header" id="home">
			...
			<table>
				...
				<tr><th>S/N</th><th>SUBJECT NAME</th><th>GIVEN TIME</th><th>SUBMISSION DATE</th><th>GET ASSIGNMENT</th></tr>
			
				<?php
					$sql = mysqli_query($con,"SELECT `assignment`.`assignment`,`assignment`.`url`,`assignment`.`subjectid`,`assignment`.`assignmentdate`,`assignment`.`subdate` FROM `assignment` INNER JOIN `subject` ON `assignment`.`subjectid` = `subject`.`id`");
					$count = 1;
					while($row = mysqli_fetch_array($sql)){
						echo "<tr><td>".$count."</td><td>".$row['assignment']."</td><td>".$row['assignmentdate']."</td><td>".$row['subdate']."</td><td><a download href='".$row['url']."'>download</a></td></tr>";
						$count++;
					}
